...
async def _run_ps_elevated(script: str, timeout: int = 60) -> str:
    import tempfile
    import time as _time
    ts = str(int(_time.time() * 1000))
    task_name = f"KINETIC_Elevated_{ts}"
    out_file = Path(tempfile.gettempdir()) / f"kinetic_elevated_out_{ts}.txt"
    ps1_file = Path(tempfile.gettempdir()) / f"kinetic_elevated_{ts}.ps1"

    wrapped = script + f'\n | Out-File -FilePath "{out_file}" -Encoding utf8 -Width 4096'
    ps1_file.write_text(wrapped, encoding="utf-8")

    tr = f'powershell -NoProfile -File "{ps1_file}"'
    create_ps = (
    ...
    proc = await asyncio.create_subprocess_exec(
        "powershell.exe", "-NoProfile", "-Command", create_ps,
        stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
    )
    await asyncio.wait_for(proc.communicate(), timeout=15)

    run_ps = f'schtasks /run /tn "{task_name}"'
    proc = await asyncio.create_subprocess_exec(
        "powershell.exe", "-NoProfile", "-Command", run_ps,
    )
    ...
    out_text = ""
    if out_file.exists():
        out_text = out_file.read_text("utf-8").strip()
        out_file.unlink(missing_ok=True)
    ps1_file.unlink(missing_ok=True)

    clean_ps = f'schtasks /delete /tn "{task_name}" /f'
    proc = await asyncio.create_subprocess_exec(
        "powershell.exe", "-NoProfile", "-Command", clean_ps,
    )
    await asyncio.wait_for(proc.communicate(), timeout=10)

    return out_text or "ERROR: No output from elevated command."
...
async def _network_dns_lookup(args: dict[str, Any], ctx: ToolContext | None) -> str:
    host = args.get("host", "").strip()
    record_type = args.get("type", "A").strip().upper()
    if not host:
        return "ERROR: 'host' parameter is required."

    ps = (
        f"$r = nslookup -type={record_type} {host} 2>$null; $r"
    )
    result = await _run_ps(ps, timeout=15)
    if result.startswith("ERROR"):
        return result
    lines = [ln for ln in result.split("\n") if ln.strip() and not ln.startswith("Server:") and not ln.startswith("Address:")]
    return "\n".join(lines) if lines else f"No {record_type} records for {host}."


async def _network_traceroute(args: dict[str, Any], ctx: ToolContext | None) -> str:
    host = args.get("host", "").strip()
    if not host:
        return "ERROR: 'host' parameter is required."

    ps = f"""
Write-Host "=== TRACEROUTE: {host} ==="
try {{
  Test-NetConnection -ComputerName "{host}" -TraceRoute -WarningAction SilentlyContinue -ErrorAction Stop |
    Select-Object -ExpandProperty TraceRoute |
    ForEach-Object {{ Write-Host "[hop] $_" }}
}} catch {{
  tracert -h 15 "{host}" 2>$null
}}
"""
    result = await _run_ps(ps, timeout=60)
    if result.startswith("ERROR"):
        return result
    return result or f"Traceroute to {host} failed."


async def _network_whois(args: dict[str, Any], ctx: ToolContext | None) -> str:
    import httpx as _hx
    domain = args.get("domain", "").strip().lower()
    if not domain:
        return "ERROR: 'domain' parameter is required."
    if "." not in domain:
        return "ERROR: Provide a full domain name (e.g., 'example.com')."

    tld = domain.rsplit(".", 1)[-1]
    rdap_urls = {
        "com": f"https://rdap.verisign.com/com/v1/domain/{domain}",
        "net": f"https://rdap.verisign.com/net/v1/domain/{domain}",
        "org": f"https://rdap.publicinterestregistry.org/rdap/org/domain/{domain}",
        "info": f"https://rdap.afilias.net/rdap/info/domain/{domain}",
        "io": f"https://rdap.nic.io/domain/{domain}",
    }

    url = rdap_urls.get(tld)
    if not url:
        return f"WHOIS via RDAP not available for .{tld}. Try the web."
    ...
    if events.get("registration"):
        lines.append(f"Created: {events['registration']}")
    if events.get("expiration"):
        lines.append(f"Expires: {events['expiration']}")
    if events.get("last change"):
        lines.append(f"Last Changed: {events['last change']}")
    ...
    if nameservers:
        lines.append(f"Name Servers ({len(nameservers)}):")
...
async def _network_bandwidth(args: dict[str, Any], ctx: ToolContext | None) -> str:
    ps = r"""
Write-Host "=== NETWORK ADAPTER STATISTICS ==="
Get-NetAdapterStatistics -ErrorAction SilentlyContinue |
  ...
  Format-Table -AutoSize -Wrap
Write-Host ""
Write-Host "=== TCP CONNECTIONS BY STATE ==="
...
"""
    ...
    if result.startswith("ERROR"):
        return result
    return result or "(no network data)"


async def _security_elevate_bot(args: dict[str, Any], ctx: ToolContext | None) -> str:
    import sys as _sys
    py = _sys.executable
    script = _sys.argv[0] if _sys.argv else ""
    ps = (
        f'Start-Process -FilePath "{py}" -ArgumentList "{script}" -Verb RunAs;'
        f" Start-Sleep 3"
    )
    proc = await asyncio.create_subprocess_exec(
    ...
    await asyncio.wait_for(proc.communicate(), timeout=10)
    return (
        "Restarting as administrator. A UAC prompt will appear - click Yes.\n"
        "After that, all commands including defender_set will work directly."
    )


async def _security_defender_set(args: dict[str, Any], ctx: ToolContext | None) -> str:
    realtime = args.get("realtime", "").strip().lower()
    antivirus = args.get("antivirus", "").strip().lower()

    if not realtime and not antivirus:
        return "ERROR: Provide at least one of 'realtime' or 'antivirus' ('on' or 'off')."
    if realtime and realtime not in ("on", "off"):
        return "ERROR: 'realtime' must be 'on' or 'off'."
    if antivirus and antivirus not in ("on", "off"):
        return "ERROR: 'antivirus' must be 'on' or 'off'."

    parts = []
    if realtime == "off":
        parts.append("Set-MpPreference -DisableRealtimeMonitoring $true -ErrorAction Stop")
    elif realtime == "on":
        parts.append("Set-MpPreference -DisableRealtimeMonitoring $false -ErrorAction Stop")
    if antivirus == "off":
        parts.append("Set-MpPreference -DisableAntivirus $true -ErrorAction Stop")
    elif antivirus == "on":
        parts.append("Set-MpPreference -DisableAntivirus $false -ErrorAction Stop")

    ps = "try { " + "; ".join(parts) + "; Write-Host 'Changes applied.' } catch { Write-Host 'ERROR: $($_.Exception.Message)' }"
    result = await _run_ps(ps, timeout=30)

    if "access denied" in result.lower() or "permission" in result.lower() or "unauthorized" in result.lower():
        alt = await _run_ps_elevated("; ".join(parts), timeout=30)
        if alt and "ERROR" not in alt:
            result = alt
        else:
            return (
                "Requires admin rights. Your account is admin, but the bot process isn't elevated.\n"
                "To fix this automatically, reply: **elevate**\n"
                "This will restart the bot as administrator (UAC prompt will appear - click Yes).\n"
                "Or manually: right-click PowerShell → Run as Administrator → run `kinetic`"
            )
    ...
    summary = []
    if realtime:
        summary.append(f"Real-time protection: {realtime}")
    if antivirus:
        summary.append(f"Antivirus engine: {antivirus}")
    return "Changes applied: " + ", ".join(summary) + "\n" + result


async def _security_remove_firewall_rule(args: dict[str, Any], ctx: ToolContext | None) -> str:
    name = args.get("name", "").strip()
    if not name:
        return "ERROR: 'name' parameter is required (exact rule display name)."
    ps = (
        f"$rule = Get-NetFirewallRule -DisplayName '{name}' -ErrorAction SilentlyContinue"
        f"\nif (-not $rule) {{ Write-Host 'ERROR: Rule \"{name}\" not found.'; exit 1 }}"
...
def create_security_tools() -> list[ToolHandler]:
    return [
        ...
        _make_handler(
            _security_remove_firewall_rule,
            "security_remove_firewall_rule",
            "Remove a Windows Firewall rule by its exact display name. Use security_firewall_rules to list names.",
            {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "Exact display name of the rule to remove"},
                },
                "required": ["name"],
            },
        ),
        ...
        _make_handler(
            _security_defender_set,
            "security_defender_set",
            "Enable or disable Windows Defender real-time protection and/or the antivirus engine. "
            "WARNING: Disabling leaves the system vulnerable. Confirm with user first.",
            {
                "type": "object",
                "properties": {
                    "realtime": {
                        "type": "string",
                        "description": "'on' to enable real-time monitoring, 'off' to disable",
                    },
                    "antivirus": {
                        "type": "string",
                        "description": "'on' to enable the antivirus engine, 'off' to disable",
                    },
                },
            },
        ),
        ...
        _make_handler(
            _security_elevate_bot,
            "security_elevate_bot",
            "Restart the bot process as Administrator. Shows a UAC prompt - user must click Yes. "
            "Use when defender_set or other admin commands fail due to permissions.",
            {"type": "object", "properties": {}},
        ),
        ...
        _make_handler(
            _network_dns_lookup,
            "network_dns_lookup",
            "Perform a DNS lookup for any record type (A, AAAA, MX, NS, TXT, CNAME, etc.). Uses nslookup.",
            {
                "type": "object",
                "properties": {
                    "host": {"type": "string", "description": "Domain or hostname to look up"},
                    "type": {"type": "string", "description": "Record type: A (default), AAAA, MX, NS, TXT, CNAME, SOA, etc."},
                },
                "required": ["host"],
            },
        ),
        _make_handler(
            _network_traceroute,
            "network_traceroute",
            "Trace the network route to a remote host. Shows each hop along the path.",
            {
                "type": "object",
                "properties": {
        ...
        _make_handler(
            _network_whois,
            "network_whois",
            "Look up domain registration information (registrar, dates, name servers) via RDAP. Supports .com, .net, .org, .info, .io.",
            {
                "type": "object",
                "properties": {
        ...
        _make_handler(
            _network_bandwidth,
            "network_bandwidth",
            "Show network adapter traffic statistics (bytes sent/received) and TCP connection state summary.",
            {"type": "object", "properties": {}},
        ),
